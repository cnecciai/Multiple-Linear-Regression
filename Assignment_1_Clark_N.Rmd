...
```{r}
CBS$int_hour_day <- interaction(CBS$hour, CBS$day)
```
...
#In order for the interaction to work, I had to remove hour and day from the model... 
#Getting an hour saying, "there are aliased variables"


#season - year - month - hour - day  - weather - day_of_month
CBS <- dummy_cols(CBS, select_columns = 
                    c("season", "year", "month", "weather", "int_hour_day"),
                  remove_first_dummy = T) %>%
  dplyr::select(-c(season, year, month, hour, day, weather, int_hour_day))
...
OLS.regback <- leaps::regsubsets(cube_root_total ~ . - count_rentals - temp - workingday , train, method = "seqrep", nvmax = 175)
...
plot(OLSregbacksum$adjr2, xlab = "Number of Variables",
    ...
    font.lab = 2, panel.first = grid(nx = NULL, ny = NULL, col = "gray",
        lty = 2))
points(171, OLSregbacksum$adjr2[171], col = "#336699", cex = 2, pch = 20)
...
points(171, OLSregbacksum$bic[171], col = "#336699", cex = 2, pch = 20)
```
