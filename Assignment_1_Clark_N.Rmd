...
There is a notable pattern in bicycle rentals during the commute hours to/from work. It may be the case that Capital Bike Share bicycles are used as a primary mode of transportation for the majority of riders during working hours.  Before 9:00AM, we note an upward trend of rentals followed by a stark decrease just after. We are interpreting this as individuals arriving at work. This is followed by high usage at 5:00PM when people typically leave work, where the bikes may be rapidly available. This is then followed with slowly decreasing usage. Based on these insights, we will be considering including an interaction term between `hour` and `day` in our models to capture this variability in rental numbers. Doing so will also allow us to capture the high usage trends during weekend hours as well. 
...
CBS[!(CBS$day %in% c("Sunday", "Saturday")),] %>% 
  ...
  geom_boxplot(alpha = 0.7, width = .8) +
  labs(x = "", y = "Total Rentals",
       title = "Weekday Hourly Trend of Rentals",
...
##### `hour`  We noted previously on the interaction between `hour` and `day` and its affects on the number of rentals. We wish to capture this discernible pattern in order to yield better model performance overall. To naively create an interaction between these two variables as they currently exist would create **168** new variables. This would undoubtedly create much overhead for our models but also runs the risk of overfitting.  

We decided on an optimal solution of re-categorizing our `hour` variable down to just five distinct categories. The resulting variables consisted of different time recognizable, common-sense periods of time during the day. Doing so allowed us to not only reduce our overall number of categories down to **35**, but still capture much of the underlying interaction between `hour` and `day`.

```{r fig.width=9, fig.height = 5}

#Create appropriate subsets of hourly categories
morning <- c("12:00AM", "1:00AM", "2:00AM", "3:00AM", "4:00AM", "5:00AM","6:00AM","7:00AM", "8:00AM","9:00AM")
midday <- c("10:00AM","11:00AM","12:00PM","1:00PM","2:00PM", "3:00PM","4:00PM","5:00PM", "6:00PM")
evening_night <- c("7:00PM", "8:00PM", "9:00PM", "10:00PM","11:00PM")
time_of_day <- rep(NA, length(CBS))

#Determine Proper Category
for (i in 1:nrow(CBS)) {
  curr_hour = CBS$hour[i]
  if (curr_hour %in% morning) {
    cat = "morning"
  } else if (curr_hour %in% midday) {
    cat = "midday"
  } else { #evening_night
    cat = "evening_night"
  }
...
#Reassign Categories to Hour Variable
CBS$hour = factor(time_of_day)
```


```{r fig.width=8, fig.height = 4}
ordering = c("early_morning", "morning", "midday", "late_afternoon", "evening_night")
ordering <- ordering %>% rev()
  
CBS %>% 
ggplot(mapping = aes(x = factor(hour, levels = ordering), count_rentals)) +
  geom_boxplot(aes(fill = workingday), alpha = 0.7, width = .8) +
  labs(x = "", y = "Total Rentals",
       title = "Categorized Hours of Total Rentals",
       subtitle = "Disparity of Rental Trends Still Present in Re-Categorized Hour Variable")+
  scale_fill_viridis_d(option = "magma") +
  theme(axis.text.x = element_text(angle = 25,
                                   size = 7),
        axis.ticks = element_line(color = "black")) +
  coord_flip()+
  facet_wrap(~day, ncol = 7, nrow  = 1)
```
...
```{r}
#CBS$hr_wk <- interaction(CBS$hour, CBS$day)
```


Or I could just do an interaction between some of the variables...
...
CBS <- dummy_cols(CBS, select_columns = 
                    c("season", "year", "month", "day", "hour", "weather", "hr_wk"),
                  remove_first_dummy = F) %>%
  dplyr::select(-c(season, year, month, day, hour, weather, hr_wk))
...
model_assumption_ver <- lm(cube_root_total ~ . - count_rentals - temp,
                          data = train)

plot(model_assumption_ver)
...
OLS.regback <- leaps::regsubsets(cube_root_total ~ . - count_rentals - temp, train, method = "seqrep", nvmax = 175)
...
plot(OLSregbacksum$adjr2, xlab = "Number of Variables",
    ...
    font.lab = 2, panel.first = grid(nx = NULL, ny = NULL, col = "gray",
        lty = 2))
points(65, OLSregbacksum$adjr2[65], col = "darkred", cex = 2, pch = 20)
...
points(65, OLSregbacksum$bic[65], col = "darkred", cex = 2, pch = 20)
```
