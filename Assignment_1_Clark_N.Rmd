...
There being **731** distinct dates could have severe impacts, mainly overfitting, when regressing our target variables. The only unique piece of information provided by `date` is the day of the month. All other information provided by `date` is captured by our `year` and `month` variable. To prevent these excessive categories from being created, we will extract the day of the month provided by the `date` variable, but eliminate the `date` variable from our analysis. 

```{r}
CBS$day_of_month <- factor(day(CBS$date))
...
CBS <- dummy_cols(CBS, select_columns = 
                    c("season", "year", "month", "hour", "day", "weather", "day_of_month"),
                  remove_first_dummy = T) %>%
  dplyr::select(-c(season, year, month, hour, day, weather, day_of_month))
...
b <- boxcox(lm(CBS$count_rentals ~ 1), plotit = F)
...
model_assumption_ver <- lm(cube_root_total ~ . - count_rentals - temp - workingday - season_Summer,
                          data = train)
...
OLS.regback <- leaps::regsubsets(cube_root_total ~ . - count_rentals - temp - workingday - season_Summer, train, method = "seqrep", nvmax = 70)
...
plot(OLSregbacksum$adjr2, xlab = "Number of Variables",
    ...
    font.lab = 2, panel.first = grid(nx = NULL, ny = NULL, col = "gray",
        lty = 2))
points(39, OLSregbacksum$adjr2[39], col = "#336699", cex = 2, pch = 20)
...
points(39, OLSregbacksum$bic[39], col = "#336699", cex = 2, pch = 20)
```
...
model_metrics_df = metrics(y_pred = pred_values_total_rentals$fit, y_true = pred_values_total_rentals$obs)
...
ggplot(pred_values_total_rentals, aes(x=obs, y=fit)) +
  geom_point(aes(color = abs(obs - fit)), size = 3, alpha = 0.6) + 
  geom_abline(intercept=0, slope=1, linetype="dashed", color="red") +
  scale_color_viridis_c(option = "A", direction = -1) + 
  labs(
    title = "Actual vs Predicted Number of Total Rentals",
    subtitle =  paste("Expected Average Error from Prediction: ", round(rmse_total_rentals,2), "Total Rentals"),
    x = "Actual Number of Total Rentals",
    y = "Predicted Number of Total Rentals",
